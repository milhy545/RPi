...
#security-banner.show{display:flex}#security-banner.warn{border-color:#d29922;background:#2d2308}#security-banner.ok{border-color:#238636;background:#0d2818;color:#3fb950}#security-banner a{font-weight:700;white-space:nowrap}
...
.info-btn{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:50%;border:1px solid #8b949e;color:#8b949e;font-size:.65rem;font-style:italic;cursor:pointer;vertical-align:middle;margin-left:.3rem;background:transparent;line-height:1;padding:0}
...
function applyLang(){document.documentElement.lang=LANG==='en'?'en':'cs';document.querySelectorAll('[data-i18n]').forEach(el=>{let key=el.dataset.i18n;let txt=L(key);if(el.dataset.i18nAttr==='placeholder'){el.placeholder=txt;return}if(el.dataset.i18nAttr==='title'){el.title=txt;return}let icon=el.dataset.icon||'';el.innerHTML=(icon?icon+' ':'')+txt});document.querySelectorAll('[data-tip]').forEach(el=>{if(el.nextElementSibling&&el.nextElementSibling.classList.contains('tip-wrap')){let box=el.nextElementSibling.querySelector('.tip-box');if(box)box.textContent=helperText(el.dataset.tip)}});document.querySelectorAll('[data-lang-btn]').forEach(btn=>btn.classList.toggle('active',btn.dataset.langBtn===LANG));let ls=$('#lang-status');if(ls)ls.textContent=LANG==='en'?'EN':'CZ';updateSecurityBanner()}
...
<div class="sec"><h3 data-i18n="diagnostics" data-tip="sectionDiagnostics">Diagnostics</h3><div id="ta-summary" class="media-meta">Click Refresh</div><details style="margin-top:.5rem"><summary class="media-meta" style="cursor:pointer">Raw technical JSON</summary><pre id="ta-raw">Click Refresh</pre></details></div>
</div>
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/report":
            ...
            if not isinstance(report, dict) or not isinstance(report.get("type"), str) or not isinstance(report.get("description"), str) or not report["description"].strip():
                return self.st(400,"Missing or invalid required fields (type must be a string, description must be non-empty)", "text/plain")
