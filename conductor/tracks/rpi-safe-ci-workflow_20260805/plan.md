...
- [x] **Phase 3: Profiles & Script Integration**
  ...
  - [x] Add E2E/evidence gates in `ci-agent.sh` requiring SHA-bound Playwright manifest and exact-SHA RPi receipt before push.
  ...
  - [x] Fix bypassable hard gates: preserve raw Playwright exit status, require TARGET_URL, fail milhy-full when Playwright missing.
  - [x] Resolve CI_PROFILE once via RESOLVED_CI_PROFILE to avoid unbound var under set -u.
  - [x] Add explicit evidence-validation / NO_PUSH mode in `ci-agent.sh`.
...
- [x] **Phase 4: RPi Core Rules Template & Idempotent Installer**
  ...
  - Create `tools/install-rpi-core-rules.sh` with timestamped backup, symlink resolution, boundary validation (`.agents/skills` and `.codex/skills`), atomic install, and rollback on failure.
  - Targets `~/.agents/skills/core-rules/SKILL.md` or `~/.codex/skills/core-rules/SKILL.md` (not ~/AGENTS.md).
...
- [x] **Phase 6: Comprehensive Test Suite & Verification**
  ...
  - [x] Add deterministic tests proving push is impossible when E2E or exact-SHA RPi evidence is absent/stale, dirty state is untouched, and profiles route correctly.
  - [x] Add tests proving git ignore behavior for canonical E2E manifests/receipts and NO_PUSH mode.
  - [ ] **PENDING**: Live E2E/HW/receipt rollout evidence must exist before marking complete.
