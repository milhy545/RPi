...
- [x] Úkol: Ověřit, že tokeny `:root` v `rpi_dashboard/static/css/theme.css`
...
- [x] Úkol: Zajistit, že media queries v `responsive.css` používají CSS
  proměnné místo hardkódovaných pixelových hodnot tam, kde je to vhodné.
- [x] Úkol: Ověřit, že `audio.css` neuvádí duplicitní barvy kolidující
  s tokeny `theme.css`.
- [x] Úkol: Zauditovat `main.css` a `themes.css` ohledně hardkódovaných
  hodnot palety, které duplikují definici tokenu bez odůvodnění; nahradit
  nebo zdokumentovat každý případ.
- [x] Ověření: Zachycení Playwright screenshot baseline před změnami;
...
- [x] Úkol: Identifikovat a odstranit globální prvky vlastněné sekcí
...
- [x] Úkol: Odstranit nebo refaktorovat odpovídající funkce v `app.js`,
...
- [x] Úkol: Ověřit, že Bluetooth tab zachovává své lokální ovládače
  (scan, pair, connect) a ztrácí pouze globální duplicity.
- [x] Ověření: Vizuální kontrola na 1920x1080 potvrzuje jeden header,
  jednu navigační lištu, jeden ovládač jazyka a jeden stavový panel.

## Fáze 3: Oprava napojení telemetrie stavového panelu

- [x] Úkol: Opravit nesoulad DOM ID: buď přejmenovat `sb-cpu`/`sb-ram`
...
- [x] Úkol: Zajistit, že pravidelné načítání `/system/hw-stats` začíná
  ...
  procesů/jader.
- [x] Úkol: Zobrazovat `--` pro CPU, RAM a teplotu, pokud je backend
  endpoint nedostupný nebo vrací chybu.
- [x] Ověření: Načtení stránky; stavový panel zobrazuje živé hodnoty
  do 2 sekund.

## Fáze 4: Oprava chyb v konzoli

- [x] Úkol: Opravit null-reference v `updBr()` přidáním null stráží
  před zápisem do prvků `brb`/`brs`, nebo funkci odstranit, pokud
  prvky po fázi 2 neexistují.
- [x] Úkol: Zauditovat `app.js` dalším null-reference rizikům na
  odstraněných prvcích; přidat stráže nebo odstranit mrtvý kód.
- [x] Ověření: Otevřít konzoli; nula chyb během 30-sekundového
  nečinné relace na kartě Player.

## Fáze 5: Shell Terminal tabu

- [x] Úkol: Přidat neprázdný Terminal panel shell do `index.html`, který
  zobrazuje uzamčený/stupňový stav a integrační smlouvu (očekávaný
  WebSocket transport, požadavek na autentizaci).
- [x] Úkol: Panel nesmí otevírat WebSocket spojení, dokud terminal/auth
...
- [x] Ověření: Terminal tab zobrazuje viditelný, neprázdný panel s textem
  integrační smlouvy.

## Fáze 6: Ověření responzivního layoutu

- [x] Úkol: Ověřit layout WebUI na 390x844 (mobil iPhone 14 Pro):
  navigace se správně sbalí, dotykové cílové plochy >= 44px, obsah
  je čitelný bez horizontálního skrolování.
- [x] Úkol: Ověřit na 768x1024 (iPad): dvou-sloupcový layout tam,
  kde je to vhodné, žádné překrývající se prvky.
- [x] Úkol: Ověřit na 1366x768 (notebook): plný layout se vejde bez
...
- [x] Úkol: Ověřit strukturu tabů v `tui.py`: Player, Audio, Bluetooth,
  Devices, Network, Terminal, System.
- [x] Úkol: Ověřit, že stavový panel TUI čte CPU/RAM/teplotu přes
  sdílený Python modul `rpi_dashboard.services.system` (přímé čtení
  z `/proc` a `/sys`), nikoli přes HTTP self-call do WebUI API.
- [x] Úkol: Ověřit, že přepínač CZ/EN v TUI kopíruje výchozí nastavení
  WebUI (`cz`) a že české řetězce v TUI vynechávají diakritiku pro
  kompatibilitu s TV tty.
- [x] Úkol: Použít barevné tokeny z `theme.css` palety v Textual CSS,
  kde framework umožňuje (hranice, aktivní stavy, barvy textu).
- [x] Ověření: Zaměřené Textual `run_test()` pytest pokrytí pro
...
- [x] Jednotný app header, jedna navigační lišta, jeden ovládač jazyka/
  tématu, jeden globální stavový panel — žádné globální prvky vlastněné
  sekcí Bluetooth duplikující app-level plochy.
- [x] Stavový panel zobrazuje živé CPU/RAM/teplotu z `/system/hw-stats`.
...
- [x] `tools/verify-done.sh` projde s platným CI potvrzením.
