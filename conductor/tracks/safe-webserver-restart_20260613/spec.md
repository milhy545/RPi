...
## Problem
The webserver currently unconditionally removes `/tmp/rpi-mpv.sock` on startup. If mpv is playing and the webserver restarts, the video may keep playing but WebUI loses control because the socket path is unlinked.

## Requirements
- Do not unlink `/tmp/rpi-mpv.sock` if a live mpv process is using it and the socket responds.
