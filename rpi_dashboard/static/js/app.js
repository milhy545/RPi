...
function renderBtDevices(devs){if(!devs||!devs.length)return '<div>'+L('noBtDevices')+'</div>';return devs.map(d=>'<div class="media-card" style="min-height:70px;margin:.3rem 0"><h4>'+devIcon(btRole(d))+' '+esc(d.name||'Unknown')+' '+badge(!!d.connected,d.connected?'CONNECTED':(d.paired?'PAIRED':'FOUND'))+'</h4><div class="media-meta">MAC: '+esc(d.mac)+'<br>Role: '+esc(btRole(d))+' · Trusted: '+(d.trusted?'yes':'no')+'</div><div class="row" style="margin-top:.35rem"><button data-i18n="pair" onclick="btPair(\''+jsarg(d.mac)+'\')">Pair</button><button data-i18n="connect" onclick="btConnect(\''+jsarg(d.mac)+'\')">Connect</button><button data-i18n="trust" onclick="btTrust(\''+jsarg(d.mac)+'\')">Trust</button><button data-i18n="disconnect" onclick="btDisconnect(\''+jsarg(d.mac)+'\')">Disconnect</button><button class="danger" data-i18n="remove" onclick="btRemove(\''+jsarg(d.mac)+'\')">Remove</button></div></div>').join('')}
function renderBtController(c){if(!c)return '—';let mods=c.modules||{},ertm=c.ertm||{},inputs=c.input_devices||[],steam=c.steamlink||{};let driver=mods.xpadneo?'xpadneo':(mods.uhid?'uhid':(mods.xpad?'xpad':'missing'));let rows=[];rows.push(badge(!!c.ready,c.ready?L('btReady'):L('btNotReady'))+' '+(c.connected||[]).length+' '+L('connected'));rows.push(L('btErtm')+': '+(ertm.disabled===true?'yes':(ertm.disabled===false?'no':'unknown')));rows.push(L('btDriver')+': '+esc(driver));rows.push(L('btInput')+': '+esc(inputs.length?inputs.join(', '):'none'));rows.push(L('btSteamLink')+': '+(steam.available?'available':'missing'));return rows.map(x=>'<div>'+x+'</div>').join('')}
function renderBluetoothState(bt){let devs=(bt&&bt.devices)||[...((bt&&bt.paired)||[]),...((bt&&bt.scanned)||[])];$('#bt-list').innerHTML=renderBtDevices(devs);$('#bt-controller').innerHTML=renderBtController(bt&&bt.controller);$('#bt-status').textContent=((bt&&bt.scanned)||[]).length+' '+L('found')+', '+((bt&&bt.paired)||[]).length+' '+L('paired');}
async function bluetoothRefresh(){let r=await api('/devices/state');if(r.error){msg(r.error,'err');return}renderBluetoothState(r.bluetooth||{});applyI18n()}
async function bluetoothScan(){msg(L('scan')+' Bluetooth...','info');let r=await api('/bt/scan?seconds=6');if(r.error){msg(r.error,'err');return}renderBluetoothState({devices:r.all||[...(r.paired||[]),...(r.devices||[])],paired:r.paired||[],scanned:r.devices||[],controller:r.controller});msg(L('scan')+' Bluetooth done','ok');applyI18n()}
