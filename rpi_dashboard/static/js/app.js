...
function renderAudioTopology(r){let srcCol=$('#audio-topo-sources'),snkCol=$('#audio-topo-sinks');if(!srcCol||!snkCol)return;let sources=(r&&r.sources)||[],sinks=(r&&r.sinks)||[],defSink=(r&&r.default_sink)||'';let srcHtml='';if(sources.length===0){srcHtml='<div class="media-meta">No active sources</div>'}else{sources.forEach(s=>{let isPriority=s.type==='usb_input'||(s.name&&s.name.includes('mpv'));let isSel=topoSelectedSource===s.name;srcHtml+='<div class="audio-node-card '+(isPriority?'priority':'')+' '+(isSel?'selected':'')+'" onclick="audioSelectSource(\''+jsarg(s.name)+'\')">';srcHtml+='<div class="audio-node-header"><span>'+(isPriority?'🎬':'🎙️')+' '+esc(shortName(s.name))+'</span><span class="badge ok">ACTIVE</span></div>';srcHtml+='<div class="audio-node-meta">Type: '+esc(s.type)+' · Vol: '+(s.volume!==null?s.volume+'%':'100%')+'</div></div>'})}srcCol.innerHTML=srcHtml;let snkHtml='';if(sinks.length===0){snkHtml='<div class="media-meta">No active sinks</div>'}else{sinks.forEach(s=>{let isDef=s.name===defSink;snkHtml+='<div class="audio-node-card '+(isDef?'selected':'')+'" onclick="audioSelectSink(\''+jsarg(s.name)+'\')">';snkHtml+='<div class="audio-node-header"><span>'+(s.type==='hdmi'?'📺':s.type==='bt'?'🔈':'📡')+' '+esc(shortName(s.name))+'</span>'+(isDef?'<span class="badge ok">DEFAULT</span>':'')+'</div>';snkHtml+='<div class="audio-node-meta">Type: '+esc(s.type)+' · State: '+esc(s.state||'IDLE')+'</div>';if(s.volume!==null){snkHtml+='<div class="audio-node-vol"><span style="font-size:0.7rem;color:var(--app-muted)">Vol:</span><input type="range" min="0" max="150" value="'+s.volume+'" onchange="taVol(\''+s.type+'\',\''+jsarg(s.name)+'\',this.value)"></div>'}snkHtml+='</div>'})}snkCol.innerHTML=snkHtml}
