...
function sw(n){$$('.tab').forEach(t=>t.classList.toggle('active',t.dataset.t===n));$$('.pnl').forEach(p=>p.classList.toggle('active',p.id==='p-'+n));if(n==='player'){playerEnter()}if(n==='bluetooth'){btInitInteractions();setTimeout(()=>{btCenterCanvas(true);btDrawTopologyLines()},120)}if(n==='terminal'){loadHwStats();loadSysStatus();if(term){setTimeout(termFitNow,80);setTimeout(termFitNow,250)}}}
...
function btNormalizeState(bt){let v2=bt&&bt.v2?bt.v2:bt||{};let devices=(v2.devices)||((bt&&bt.devices)||[...((bt&&bt.paired)||[]),...((bt&&bt.scanned)||[])]);return{raw:v2,devices:devices||[],adapters:v2.adapters||[],backend:v2.backend||{},diagnostics:v2.diagnostics||{},operations:v2.operations||[],events:v2.events||[]}}
...
function renderBtQuick(){let items=[['plus-circle','Párovat<br>Nové','btPairNew()',''],['arrows','Obnovit<br>Topo','bluetoothRefresh()',''],['export','Export<br>Dat','btExportData()',''],['priority','Změnit<br>Prioritu','btAdapterPriority()',''],['leaf','Úspora<br>Energie','btPowerSave()','green'],['connect','Připojit<br>Vše','btConnectAll()',''],['x','Odpojit<br>Vše','btDisconnectAll()','red'],['gear','Další<br>Nastavení','btMoreSettings()','']];return items.map(i=>'<button type="button" class="bt-action-tile '+i[3]+'" onclick="'+i[2]+'"><b>'+i[0].slice(0,1).toUpperCase()+'</b><span>'+i[1]+'</span></button>').join('')}
...
function renderBtDeviceDetails(devs,adapters){let d=btSelectedDevice();if(!d)return '<div class="bt-muted">No device selected.</div>';let a=(adapters||[]).find(x=>x.id===d.adapter_id),offline=d.present===false||(!d.connected&&d.paired);return '<div class="bt-detail-title">'+esc(d.name||d.alias||'Unknown Device')+'</div><div class="bt-status-pill '+(offline?'offline':'')+'">● '+btStatusText(d)+'</div><div class="bt-detail-box"><div class="bt-detail-row"><span>Signal</span><b class="bt-blue">'+esc(btRssi(d))+'</b></div><div class="bt-detail-row"><span>MAC Address</span><code>'+esc(btDeviceMac(d)||'-')+'</code></div><div class="bt-detail-row"><span>Adapter</span><span class="bt-adapter-tag">'+esc(a?btAdapterName(a,adapters.indexOf(a)):'-')+'</span></div><div class="bt-detail-row"><span>Battery</span><b>'+esc(d.battery_percentage==null?'N/A':d.battery_percentage+'%')+'</b></div></div><div class="bt-detail-actions"><button type="button" onclick="btSelectedAction(\'pair\')">Pair</button><button type="button" class="danger" onclick="btSelectedAction(\'disconnect\')">Odpojit Zařízení</button><button type="button" onclick="btMoveAdapter()">Přesunout adaptér</button><button type="button" onclick="btSelectedAction(\'connect\')">Connect</button><button type="button" onclick="btSelectedAction(\'trust\')">Trust</button><button type="button" class="danger" onclick="btSelectedAction(\'remove\')">Remove</button></div>'}
function renderBtControls(s){let backend=s.backend||{},degraded=backend.degraded?' <span class="bt-warn">degraded</span>':'';return '<label class="bt-control-row"><span>Automatické připojení</span><span class="bt-switch"><input id="bt-auto-connect" type="checkbox" checked onchange="btToggleAutoConnect(this.checked)"><span class="bt-slider"></span></span></label><label class="bt-control-row"><span>Viditelnost sítě (All)</span><span class="bt-switch"><input id="bt-discoverable-all" type="checkbox" checked onchange="btToggleDiscoverable(this.checked)"><span class="bt-slider"></span></span></label><div class="bt-control-row"><span>Časovač</span><select id="bt-timeout" onchange="btSettingChanged(\'Timeout\',this.value)"><option>2 min</option><option>5 min</option><option>Trvale</option></select></div><div class="bt-control-row"><span>Režim skenování</span><select id="bt-scan-mode" onchange="btSettingChanged(\'Scan mode\',this.value)"><option>Balanced</option><option>Aggressive</option></select></div><div class="bt-control-row"><span>Backend</span><b>'+esc(backend.name||'legacy')+degraded+'</b></div><div class="bt-control-row"><span>Adapters</span><b>'+s.adapters.length+' · Devices '+s.devices.length+'</b></div>'}
...
async function btPower(adapter,onoff){let r=await api('/bt/adapter-power?adapter_id='+encodeURIComponent(adapter)+'&powered='+(onoff==='on'?'1':'0'));msg(r.result||r.error,r.ok?'ok':'err');setTimeout(bluetoothRefresh,800)}
async function btDeviceAction(action,adapter,key,mac){if(!adapter||!key){if(!mac){msg('No selected Bluetooth device','err');return}msg('Adapter context missing, using legacy '+action,'info')}let url=adapter&&key?('/bt/device-action?action='+encodeURIComponent(action)+'&adapter_id='+encodeURIComponent(adapter)+'&device_key='+encodeURIComponent(key)):('/bt/'+action+'?mac='+encodeURIComponent(mac));msg(action+' '+(mac||key)+'...','info');let r=await api(url);msg(r.result||r.error,r.ok?'ok':'err');setTimeout(bluetoothRefresh,1200)}
...
function btToggleAutoConnect(on){msg('Auto Connect '+(on?'enabled':'disabled'),'info')}
function btToggleDiscoverable(on){msg('Discoverable '+(on?'enabled':'disabled')+' for visible adapters','info');if(on)bluetoothScan()}
function btSettingChanged(name,value){msg(name+': '+value,'info')}
...
function btAdapterPriority(){msg('Adapter priority panel is planned; current view keeps Adapter A/B stable from backend order.','info')}
function btPowerSave(){msg('Power save preference toggled for UI; backend support is planned.','info')}
function btMoreSettings(){msg('More Bluetooth settings are planned for this route.','info')}
function btMoveAdapter(){msg('Move Adapter requires backend adapter-priority support; no device was changed.','info')}
...
function btCenterCanvas(force){let w=$('#bt-topo-wrapper'),c=$('#bt-topo-canvas');if(!w||!c)return;if(!force&&BT_UI.panX!==0)return;let r=w.getBoundingClientRect();if(force){BT_UI.scale=Math.min(1,Math.max(.55,r.width/1400,r.height/620))}BT_UI.panX=(r.width-(1400*BT_UI.scale))/2;BT_UI.panY=(r.height-(600*BT_UI.scale))/2;btUpdateTopoTransform()}
...
function btZoomTopo(amount){BT_UI.scale=Math.min(Math.max(.45,BT_UI.scale+amount),2);btUpdateTopoTransform();btDrawTopologyLines()}
