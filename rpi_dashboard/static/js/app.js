...
function devIcon(k){return k==='xbox_controller'||k==='gamepad'?'🎮':(k==='speaker'||k==='audio_output'?'🔊':(k==='input'?'⌨️':'📡'))}
function btRole(d){return d.kind||d.type||'unknown'}
function btDeviceKey(d){return d.key||d.device_key||''}
function btDeviceMac(d){return d.address||d.mac||''}
function btActionUrl(action,d){let key=btDeviceKey(d),adapter=d.adapter_id||'';if(key&&adapter)return '/bt/device-action?action='+encodeURIComponent(action)+'&adapter_id='+encodeURIComponent(adapter)+'&device_key='+encodeURIComponent(key);return '/bt/'+action+'?mac='+encodeURIComponent(btDeviceMac(d))}
function renderBtAdapters(bt){let adapters=(bt&&bt.adapters)||[];if(!adapters.length)return '<div class="media-meta">No Bluetooth adapters present.</div>';return adapters.map(a=>{let state=a.present?(a.powered?'POWERED':'OFF'):'ABSENT';let scan=a.discovering?'DISCOVERING':'IDLE';let role=a.role?(' · '+esc(a.role)):'';let h='<div class="media-card" style="min-height:108px"><h4>📡 '+esc(a.alias||a.name||a.id)+' '+badge(!!a.present&&!!a.powered,state)+'</h4>';h+='<div class="media-meta">ID: '+esc(a.id)+'<br>Address: '+esc(a.address||'Unknown')+'<br>Path: '+esc(a.bluez_path||'—')+' / hci'+(a.index==null?'?':esc(String(a.index)))+role+'<br>Discovery: '+esc(scan)+'</div>';h+='<div class="row" style="margin-top:.4rem"><button onclick="btDiscovery(\''+jsarg(a.id)+'\',\''+(a.discovering?'stop':'start')+'\')">'+(a.discovering?'Stop scan':'Scan')+'</button><button onclick="btPower(\''+jsarg(a.id)+'\',\''+(a.powered?'off':'on')+'\')">'+(a.powered?'Power off':'Power on')+'</button></div></div>';return h}).join('')}
function renderBtDevices(devs,adapters){if(!devs||!devs.length)return '<div>'+L('noBtDevices')+'</div>';let names={};(adapters||[]).forEach(a=>names[a.id]=a.alias||a.name||a.id);return devs.map(d=>{let mac=btDeviceMac(d),key=btDeviceKey(d),adapter=d.adapter_id||'',status=d.connected?'CONNECTED':(d.paired?'PAIRED':(d.present===false?'ABSENT':'FOUND'));let signal=d.rssi==null?'Unknown':d.rssi+' dBm';let battery=d.battery_percentage==null?'Unknown':d.battery_percentage+'%';let h='<div class="media-card" style="min-height:94px;margin:.3rem 0"><h4>'+devIcon(btRole(d))+' '+esc(d.name||d.alias||'Unknown')+' '+badge(!!d.connected,status)+'</h4>';h+='<div class="media-meta">Adapter: '+esc(names[adapter]||adapter||'Unknown')+'<br>MAC: '+esc(mac)+'<br>Role: '+esc(btRole(d))+' · Paired: '+(d.paired?'yes':'no')+' · Trusted: '+(d.trusted?'yes':'no')+' · Services: '+(d.services_resolved===true?'resolved':(d.services_resolved===false?'unresolved':'unknown'))+'<br>RSSI: '+esc(signal)+' · Battery: '+esc(battery)+'</div>';h+='<div class="row" style="margin-top:.35rem"><button onclick="btDeviceAction(\'pair\',\''+jsarg(adapter)+'\',\''+jsarg(key)+'\',\''+jsarg(mac)+'\')">Pair</button><button onclick="btDeviceAction(\'connect\',\''+jsarg(adapter)+'\',\''+jsarg(key)+'\',\''+jsarg(mac)+'\')">Connect</button><button onclick="btDeviceAction(\'trust\',\''+jsarg(adapter)+'\',\''+jsarg(key)+'\',\''+jsarg(mac)+'\')">Trust</button><button onclick="btDeviceAction(\'untrust\',\''+jsarg(adapter)+'\',\''+jsarg(key)+'\',\''+jsarg(mac)+'\')">Untrust</button><button onclick="btDeviceAction(\'disconnect\',\''+jsarg(adapter)+'\',\''+jsarg(key)+'\',\''+jsarg(mac)+'\')">Disconnect</button><button class="danger" onclick="btDeviceAction(\'remove\',\''+jsarg(adapter)+'\',\''+jsarg(key)+'\',\''+jsarg(mac)+'\')">Remove</button></div></div>';return h}).join('')}
function renderReadiness(r){if(!r)return '—';let steps=r.steps||[];if(!steps.length)return badge(!!r.ready,r.ready?L('btReady'):L('btNotReady'));return steps.map(s=>'<div>'+badge(s.state===true,s.state===true?'OK':(s.state===false?'BLOCKED':'UNKNOWN'))+' '+esc(s.label||s.id)+': '+esc(s.reason||'')+'</div>').join('')}
function renderBtController(c){if(!c)return '—';let mods=c.modules||{},inputs=c.input_devices||[],steam=c.steamlink||{},blockers=c.blockers||[];let driver=mods.xpadneo?'xpadneo':(mods.uhid?'uhid':(mods.xpad?'xpad':(mods.hid_microsoft?'hid_microsoft':'missing')));let rows=[];rows.push(badge(!!c.ready,c.ready?L('btReady'):L('btNotReady'))+' Controllers: '+((c.controllers||c.connected||[]).length));rows.push(L('btDriver')+': '+esc(driver));rows.push(L('btInput')+': '+esc(inputs.length?inputs.join(', '):'unknown'));rows.push(L('btSteamLink')+': '+(steam.available===true?'available':(steam.available===false?'missing':'unknown')));if(blockers.length)rows.push('Blockers: '+esc(blockers.join(', ')));return rows.map(x=>'<div>'+x+'</div>').join('')}
...
function renderBluetoothState(bt){let v2=bt&&bt.v2?bt.v2:bt;let devs=(v2&&v2.devices)||((bt&&bt.devices)||[...((bt&&bt.paired)||[]),...((bt&&bt.scanned)||[])]);let adapters=(v2&&v2.adapters)||[];$('#bt-adapters').innerHTML=renderBtAdapters(v2||{});$('#bt-list').innerHTML=renderBtDevices(devs,adapters);$('#bt-controller').innerHTML=renderBtController((v2&&v2.diagnostics&&v2.diagnostics.controllers)||(bt&&bt.controller));$('#bt-soundbar').innerHTML=renderReadiness(v2&&v2.diagnostics&&v2.diagnostics.soundbar);$('#bt-events').innerHTML=renderBtEvents(v2||{});let backend=(v2&&v2.backend)||{};$('#bt-status').textContent='Backend: '+(backend.name||'legacy')+(backend.degraded?' (degraded)':'')+' · adapters: '+adapters.length+' · devices: '+(devs?devs.length:0);}
async function bluetoothRefresh(){let r=await api('/bt/state');if(r.error&&!r.backend){msg(r.error,'err');return}renderBluetoothState(r);applyI18n()}
