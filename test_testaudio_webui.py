...
def main() -> int:
    ...
    failures += check("Player has paste button inside URL input wrapper", 'class="url-wrap"' in html and 'class="url-paste"' in html and "onclick=\"pasteClipboardUrl()\"" in html and "function pasteClipboardUrl" in html)
    failures += check("Player paste button has no duplicate icon/text and no manual preview button", "📋 Paste" not in html and "data-icon=\"📋\"" not in html and "🖼 Preview" not in html)
    failures += check("Player default quality is 720p", '<option value="720p" selected>720p</option>' in html)
